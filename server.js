...
const { GoogleGenerativeAI } = require("@google/generative-ai"); // This might not be needed anymore
...
// Quiz route (assuming this requires no authentication)
...
async function generateStrategy(userInput) {
  try {
    ...
    const genAI = new GoogleGenerativeAI(apiKey); // Might need to be removed if not used elsewhere
...
// Logout route (no longer needed since no authentication)
